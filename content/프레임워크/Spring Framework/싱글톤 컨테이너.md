...
> [!NOTE] 실행 환경
> 버전 명시 없음 — `@Configuration`/`@Bean` 등 Spring 표준 API만 사용되어 특정 버전은 확정하기 어렵다.
...
- [(학습/프레임워크/Spring Framework) Bean ( @Scope , Provider )](Bean%20(%20@Scope%20,%20Provider%20).md) — 싱글톤의 상태 관리 문제를 @Scope와 Provider로 해결하는 방법을 다루는 노트
- [(학습/프레임워크/Spring Framework) Spring 컨테이너](Spring%20컨테이너.md) — 이 노트에서 설명하는 싱글톤 관리 주체인 Spring 컨테이너 자체의 종류와 계층 구조를 다루는 노트
