...
`[5, 3, 8, 4, 2, 7, 1, 6]`, pivot = 마지막 값(6)이라고 할 때:

```text
[5, 3, 8, 4, 2, 7, 1, 6]   start=0, end=6, pivot=6
 ->  5,3,4,2,1 (pivot보다 작음) | 8,7 (pivot보다 큼)
 ->  [5, 3, 4, 2, 1, 6, 8, 7]   pivot(6)이 제자리로 이동
 ->  왼쪽 [5, 3, 4, 2, 1] 과 오른쪽 [8, 7] 에서 각각 pivot을 새로 뽑아 재귀 반복
```

이렇게 pivot을 기준으로 "작은 값 묶음 | pivot | 큰 값 묶음"으로 나누는 과정을 분리된 집합이 1개 이하가 될 때까지 재귀적으로 반복하면 정렬이 완료된다.
...
---
## 관련 문서
- [(퀵정렬) 백준 11004번]([퀵정렬]%20백준%2011004번.md) — 퀵정렬의 partition 개념을 응용해 K번째 수를 구하는 문제
