...
*   문제에서 숫자의 최대크기를 제한했으므로, 메모리를 해당 수 이하만큼 돌면 되므로 max_size를 설정하여 기수정렬을 풀이하면된다.
    *   getMax()로 배열의 최대값을 구해 자릿수를 계산하는 방식은 배열 전체를 한 번 더 순회해야 하므로 시간 초과가 발생한다. 문제 조건에서 수의 최대값이 10,000(5자리)임을 알 수 있으므로, max_size를 5로 고정해 불필요한 순회를 없애야 한다.
...
---
## 관련 문서
- [[정렬] 기수정렬]([정렬]%20기수정렬.md) — 이 문제에 적용된 기수정렬 개념 설명
