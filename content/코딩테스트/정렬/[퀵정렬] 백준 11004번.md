...
즉, 매 partition 이후 pivot이 자리 잡은 index를 K와 비교해서, 전체를 다시 정렬할 필요 없이 K가 속한 절반만 재귀적으로 좁혀나가면 된다.

```text
pivot index == K-1   → 바로 종료 (그 값이 정답)
pivot index >  K-1   → 왼쪽 구간만 재귀
pivot index <  K-1   → 오른쪽 구간만 재귀
```
...
---
## 관련 문서
- [[정렬] 퀵 정렬]([정렬]%20퀵%20정렬.md) — 이 문제에 적용된 퀵정렬(partition) 개념 설명
