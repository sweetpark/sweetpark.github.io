...
- [(SQL) 파티션 작업 - 핵심 개념 및 특징 정리](./[SQL]%20파티션%20작업%20-%20핵심%20개념%20및%20특징%20정리.md) — 이 프로시저 문법(DECLARE, PREPARE/EXECUTE)이 실제로 쓰이는 사례(파티션 자동 순환 프로시저)
- [(MySQL) 이벤트처리 - 핵심 개념 및 특징 정리](./[MySQL]%20이벤트처리%20-%20핵심%20개념%20및%20특징%20정리.md) — 이 프로시저를 주기적으로 호출하는 `CREATE EVENT ... DO CALL` 스케줄러 문법을 다루는 자매 노트
