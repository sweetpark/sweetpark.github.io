...
- [(아키텍처) 점진적 인증 전환과 게이트웨이 책임분리 패턴](../아키텍처·설계/[HTTP]%20점진적%20인증%20전환과%20게이트웨이%20책임분리%20패턴%20-%20핵심%20개념%20및%20특징%20정리.md) — 외부 게이트웨이 연동 시 "라우팅과 결과코드 체계만 내부 표준에 맞춘다"는 원칙으로 이 결과코드 중앙화 설계를 실제 게이트웨이 클라이언트 설계에 적용하는 사례
