...
- [(게시판 프로젝트) [리팩터링] 게시판 프로젝트 아키텍처]([리팩터링]%20게시판%20프로젝트%20아키텍처.md) — 이 검증 로직이 속한 게시판 프로젝트의 전체 아키텍처 개요
- [(게시판 프로젝트) [트러블 슈팅]정상적인 update() 불가 (@InitBinder)]([트러블%20슈팅]정상적인%20update()%20불가%20(@InitBinder).md) — 여기서 도입한 @InitBinder + bindingResultValidate 패턴이 실제로 update() 오류를 유발한 트러블슈팅 사례
