...
- **스레드(Thread)**: 컴퓨터가 하나의 작업을 처리하는 가장 작은 단위
- **프로세스(Process)**: 하나의 프로그램이 실행되는 단위

이 두 개념은 크기(단위)는 다르지만, 결국 "컴퓨터가 작업을 처리하는 방식"과 관련되어 있기 때문에 항상 함께 비교됩니다.

### 컴퓨터가 "작업을 처리한다"는 것의 의미

컴퓨터는 저장된 데이터를 활용하거나 새로운 데이터를 입력받아 연산을 수행한 뒤, 결과를 도출하는 역할을 합니다. 이러한 일련의 과정을 우리는 "컴퓨터가 작업을 처리한다"라고 표현합니다. 이때, 컴퓨터가 이 작업을 **프로세스**로 처리할지, 아니면 **스레드**로 처리할지 결정해야 합니다.

### 언제 멀티스레드/멀티프로세스를 고려하는가
...
## 프로세스 vs 스레드 (OS의 스케줄링 관점)

현대의 컴퓨터는 하나의 CPU 안에 여러 개의 코어를 포함하고 있습니다. 코어가 여러 개인 이유는 단순히 하나의 코어 성능을 높이는 것보다, 여러 개의 코어를 병렬로 활용하는 것이 비용 대비 성능이 뛰어나기 때문입니다.

운영체제(OS)는 여러 개의 프로세스와 스레드를 적절히 배분하여 컴퓨터가 원활하게 동작하도록 관리합니다. OS가 프로세스를 관리하는 방식까지 깊이 이해할 필요는 없지만, 기본적인 원리는 알고 있어야 합니다. 운영체제는 CPU를 여러 프로세스와 스레드에 번갈아 가며 할당하는데, 이 과정에서 **Context Switching**이라는 개념이 등장합니다.

### Context Switching이란?

**Context Switching**은 CPU가 실행 중인 프로세스를 변경할 때 발생하는 작업 전환 과정을 의미합니다.
...
+----------------------+
|      Stack           |  <- 스레드마다 독립적으로 소유
+----------------------+
|      Heap            |  <- 프로세스 내 스레드가 공유
+----------------------+
|      Data            |  <- 전역/static 변수, 스레드가 공유
+----------------------+
|      Code            |  <- 실행 명령어, 스레드가 공유
+----------------------+

스레드는 Stack만 각자 갖고, Code/Data/Heap은 같은 프로세스 내
다른 스레드들과 공유한다.
```

간단히 설명하자면:

- **Code / Data**: 프로그램이 구동되면 고정적으로 들어오는 불변의 영역이다. 컴퓨터가 프로그램의 명령을 가져다 쓰려면 code가 필요하고, 전역변수·static 변수들은 프로그램 구동 시 항상 생성되어 있어야 하기에 프로세스가 시작될 때 로드된다.
- **프로세스**는 이 메모리를 각자 독립적으로 관리하고, Context Switching이 일어날 때마다 메모리에 들어있는 값들을 새롭게 불러와야 하는 비용이 든다.
- **스레드**는 stack을 제외한 code/data/heap 데이터를 공유한다. 이로써 메모리를 새롭게 로드해야 하는 문제는 없지만, 공유되는 자원을 잘 관리해야 한다는 위험성이 존재한다. 불변객체와 코드값이라면 상관없지만, 전역변수와 동적할당 변수는 공유가 가능하기에 적절한 기술로 관리해줘야 한다.
- 대표적인 관리 기법으로는 lock, 스레드 로컬 복사(새로운 객체로 생성), 불변객체 등이 있으며 **"Thread Safe"** 라는 키워드로 더 자세히 찾아볼 수 있다.
