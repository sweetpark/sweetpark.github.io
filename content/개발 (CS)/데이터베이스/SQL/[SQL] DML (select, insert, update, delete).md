...
*   **왜 결과 순서가 예제 테이블과 다를 수 있는가**: 관계형 모델에서 테이블(릴레이션)은 원래 순서가 없는 튜플의 집합이다. `ORDER BY`를 명시하지 않으면 DB가 내부적으로 편한 순서(저장 순서, 인덱스 스캔 순서 등)로 반환할 뿐, 특정 순서를 보장하지 않는다. 아래 INSERT/UPDATE/DELETE 예시의 결과 표도 실제로는 순서가 보장되지 않으며, 이해를 돕기 위한 예시일 뿐이다.
...
*   **왜 위험한가**: WHERE 절은 "어떤 행에 적용할지"를 결정하는 필터다. 생략하면 테이블의 모든 행이 대상이 되므로, 운영 DB에서 WHERE 없이 UPDATE/DELETE를 실행하는 것은 대표적인 사고 원인 중 하나다 (먼저 같은 조건으로 SELECT를 돌려 영향받을 행을 확인하는 습관이 안전하다).
