...
이 노트는 C++의 값 타입 변환과 참조 타입 변환의 차이, 업캐스팅/다운캐스팅, 암시적/명시적 변환, 그리고 연관 없는 클래스 사이·상속 관계 클래스 사이의 타입 변환을 정리한다. 핵심은 상속 관계에서 자식→부모 변환(값·참조 모두)은 1:1로 대응되어 항상 가능하지만, 부모→자식 값 변환은 자식에만 있는 멤버 때문에 캐스팅이 불가능하고, 참조로 강제 변환하면 컴파일은 되어도 메모리 침범 위험이 있다는 점이다.
