...
이 노트는 C++ 클래스의 기본 구성과 기본 생성자·타입 변환 생성자·복사 생성자·기타 생성자, 그리고 소멸자를 정리한다. 특히 복사 생성자는 멤버 변수에 동적할당된 포인터가 있을 경우 주소와 값을 그대로 복사하기 때문에, 한 객체를 수정하면 복사본에도 영향을 주는 문제가 생길 수 있다는 점을 주의사항으로 짚는다.
