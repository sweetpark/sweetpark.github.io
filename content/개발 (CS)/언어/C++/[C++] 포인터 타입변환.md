...
이 노트는 C++ 포인터의 타입 변환을 다룬다. 단순 포인터 선언은 생성자/소멸자를 호출하지 않지만 `new`로 생성하면 호출된다는 점, 연관 없는 클래스 사이의 포인터 변환은 캐스팅은 되지만 메모리 침범 위험이 있다는 점을 짚고, 마지막으로 상속 관계에서 부모 포인터로 자식 객체를 `delete`할 때 부모 소멸자가 `virtual`이 아니면 자식 소멸자가 호출되지 않는 문제가 있으므로 최상위 클래스의 소멸자는 항상 `virtual`로 선언하는 습관을 들여야 한다는 결론을 강조한다.
