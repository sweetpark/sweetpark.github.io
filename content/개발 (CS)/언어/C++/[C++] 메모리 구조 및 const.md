...
이 노트는 프로그램의 메모리 구조(코드·데이터·힙·스택 영역)와 `const` 키워드를 정리한다. `const`는 값을 고정해 가독성과 유지보수성을 높여주지만, 흔히 예상하는 것과 달리 `const` 변수가 항상 읽기 전용 영역(.rodata)에 들어가는 것은 C++ 표준에 명시되어 있지 않으며 컴파일러마다 처리 방식이 다르다는 점이 이 노트의 핵심 포인트다.
