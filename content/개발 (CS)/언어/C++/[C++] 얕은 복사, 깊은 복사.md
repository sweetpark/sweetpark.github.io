...
이 노트는 C++의 얕은 복사와 깊은 복사를 다룬다. 컴파일러가 자동 생성하는 얕은 복사는 포인터 멤버의 주소값만 그대로 복사해 한 객체의 포인터 멤버를 수정하면 복사본에도 영향을 주는 문제가 생기며, 이를 막기 위해 복사 생성자와 복사 대입 연산자에서 명시적으로 새 메모리를 할당해 값을 복사하는 깊은 복사를 구현해야 한다. 이어서 상속 관계에서 복사 생성자/대입 연산자가 암시적으로 처리될 때와 명시적으로 부모 클래스의 복사 로직을 호출해줘야 할 때의 차이도 예제로 정리한다.
