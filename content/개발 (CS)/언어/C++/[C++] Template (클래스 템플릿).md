...
이 노트는 C++ 클래스 템플릿의 기본 선언, 특정 타입에 대한 템플릿 특수화, 그리고 타입과 값을 함께 받는 다중 인자 템플릿을 다룬다. 클래스 템플릿을 쓰면 인스턴스 생성 시 지정한 타입(`<int>`, `<double>` 등)에 맞춰 동일한 클래스 정의를 재사용할 수 있고, 특수화를 통해 특정 타입에서만 다르게 동작하도록 만들 수 있다.
