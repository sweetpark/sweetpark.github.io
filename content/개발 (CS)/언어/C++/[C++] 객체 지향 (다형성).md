...
이 노트는 C++ 객체지향의 다형성 개념과 이를 구현하는 오버로딩·오버라이딩·순수 가상함수를 정리한다. 오버로딩은 같은 이름의 함수를 인자 형태로 구분해 재정의하는 것이고, 오버라이딩은 `virtual`과 `override` 키워드를 통해 동일한 인터페이스로 객체마다 다르게 동작하도록 만드는 것이며, 순수 가상함수(`= 0`)를 하나라도 가진 클래스는 추상 클래스가 되어 인스턴스를 생성할 수 없다는 점이 핵심이다.
