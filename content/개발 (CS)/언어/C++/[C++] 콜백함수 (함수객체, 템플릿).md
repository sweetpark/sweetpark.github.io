...
이 노트는 콜백 함수를 함수 포인터, 함수 객체(Functor), 템플릿+함수 객체 순서로 발전시켜가며 각 방식의 한계를 정리한다. 함수 포인터는 상태를 저장할 수 없고, 함수 객체는 상태는 저장할 수 있지만 템플릿 없이는 한 가지 함수객체 타입만 받을 수 있어, 템플릿과 함수 객체를 결합해야 다양한 조건으로 콜백을 넘길 수 있다는 결론에 도달하며, 다만 기능이 많아질수록 템플릿으로 인한 성능 저하가 있을 수 있다는 점도 함께 짚는다.
