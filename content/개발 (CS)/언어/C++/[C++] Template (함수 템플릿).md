...
이 노트는 C++ 함수 템플릿의 기본 형태, 여러 타입을 받는 다중 템플릿 인자, 그리고 함수 템플릿 특수화를 정리한다. 함수 템플릿은 코드 재사용성과 타입 안정성을 높여주지만 복잡성과 컴파일 시간 증가라는 트레이드오프가 있으며, 특정 인자 타입(예: 사용자 정의 클래스)에 대해서만 다른 동작을 하도록 특수화할 수 있다.
