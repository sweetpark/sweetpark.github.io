...
이 노트는 C++의 다중 포인터(이중 포인터)와 다차원 배열을 다룬다. 단일 포인터로 인자를 넘기면 함수 내부에서 지역 복사본만 바뀌어 호출자의 원본 포인터는 그대로지만, 이중 포인터(`**`)로 넘기면 포인터가 가리키는 대상 자체를 바꿀 수 있다는 차이가 핵심이며, 이어서 2차원 배열을 `int (*pp)[4]` 같은 포인터로 표현하는 방법을 정리한다.
