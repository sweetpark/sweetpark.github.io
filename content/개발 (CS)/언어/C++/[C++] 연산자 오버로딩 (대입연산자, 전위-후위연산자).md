...
이 노트는 C++ 연산자 오버로딩의 전역/멤버 함수 형태, 대입 연산자 오버로딩, 그리고 전위·후위 증가 연산자 오버로딩을 정리한다. 대입 연산자를 오버로딩할 때 클래스에 포인터 멤버가 있으면 컴파일러가 자동 생성하는 기본 대입은 얕은 복사라 문제가 되므로 직접 깊은 복사를 구현해야 하며, 자원을 그대로 옮기고 원본을 비우는 이동 대입 연산자로 성능을 아낄 수도 있다는 점을 예제로 보여준다.
