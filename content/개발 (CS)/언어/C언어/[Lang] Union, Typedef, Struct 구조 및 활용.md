...
> [!TIP]
> **실무 구조 설계 팁: 공통 필드는 Struct에, 가변 필드는 Union에**
> Union 내부의 모든 멤버는 **동일한 시작 주소(Offset 0)를 겹쳐서 공유**하므로 여러 멤버를 동시에 쓰는 것은 물리적으로 불가능합니다.
> 따라서 모든 경우에 항상 유지되어야 하는 공통 데이터(예: ID, 타임스탬프, 판별 태그)는 반드시 바깥 `struct` 멤버로 선언하고, 조건에 따라 '택 1'로 바뀌는 데이터만 `union` 내부로 넣어야 데이터 오염을 막을 수 있습니다.
