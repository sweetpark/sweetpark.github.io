...
> 고민할 건 딱 네 가지다 — **어떤 타입? P 가 M/O/C 중 무엇? 길이는 고정인가 가변인가?**
> 단, **C 가 여러 개이면 먼저 "동시에 올 수 있나"를 묻는다** — 아니라면 **tagged union** (2-9).
...
| `upf_info` | object | **C** | `nf_type == UPF` | — | UPF 전용 정보 |
...
> [!IMPORTANT] P(Presence) 열이 제일 중요하다 — **M / O / C**
> **M(Mandatory)** = 없으면 에러. **O(Optional)** = 없어도 정상.
> **C(Conditional)** = **조건이 참이면 M, 거짓이면 없음.** 조건 문장은 **조건 열에 따로** 옮겨 적는다.
> 이 구분을 안 하고 코드를 짜면, "보낸 적 없는 필드가 0으로 들어와서" 생기는
> **원인을 알 수 없는 버그**가 나온다. (아래 2장에서 해결 — O 는 2-2~2-5, **C 는 2-6~2-10**)
...
## 2. Optional(O) 과 Conditional(C) — 제일 중요한 부분
...
### 2-6. C (Conditional) — 규격서 표에는 M/O 말고 **C** 가 있다

3GPP 같은 규격서의 필드 표에는 **P 열**(Presence)이 있고, 값이 세 가지다.

| P | 뜻 | 코드 표현 | `validate` 가 하는 일 |
| :---: | :--- | :--- | :--- |
| **M** | 항상 있어야 함 | 플래그 없음 | 없으면 에러 |
| **O** | 있어도, 없어도 됨 | `has_` | **있을 때만** 범위 검사 |
| **C** | **조건이 참이면 M, 거짓이면 없음** | `has_` + **조건식** | 조건 참인데 없으면 에러 / 조건 거짓인데 있으면 **정책대로** |

C 는 표만 봐서는 안 되고 **설명 열의 조건 문장**을 읽어야 한다.
("`nfType` 이 `SMF` 이면 present", "`notifEnable` 이 true 이면 M")
그리고 이 조건 문장이 **어떤 모양이냐**가 코드 구조를 정한다 — 아래 2-8.

> [!IMPORTANT] C 를 O 처럼 처리하면 **검사가 통째로 빠진다**
> O 는 "없어도 정상"이라 `validate` 가 아무것도 안 한다. C 는 **"어떤 경우엔 없으면 안 된다."**
> 표에서 C 를 O 로 옮겨 적으면, 필수여야 할 필드가 빠진 요청이 **정상으로 통과**한다.
> 0장의 표 ①에 **조건 열**을 하나 더 만들어 두는 이유다.

### 2-7. C 가 **하나뿐**일 때 — `has_` + 조건 검사

구조체는 O 와 똑같다. **달라지는 건 `validate` 뿐**이다.

**① 규격서의 조건 문장을 표로 먼저 옮긴다** (코드 치기 전)

| 필드 | 조건 | 조건 참일 때 | 조건 거짓일 때 |
| :--- | :--- | :--- | :--- |
| `notif_uri` | `notif_enable == 1` | **필수** | 없어야 정상 (있으면 → 정책) |

**② 그 표가 그대로 코드가 된다**

```c
/* 조건식은 함수로 뽑는다 — validate 본문이 조건 문장으로 어지러워지지 않게 */
static int cond_notif_uri_required(const sub_req_t *r)
{
    return r->notif_enable != 0;
}

/* validate 안에서 */
if (cond_notif_uri_required(r)) {
    if (!r->has_notif_uri) {                     /* ★ 조건 참인데 없다 → 에러 */
        LOG_ERR("notif_uri required when notif_enable=1 (%s/%d)", __func__, __LINE__);
        return RET_INVALID_ARG;
    }
} else if (r->has_notif_uri) {                   /* 조건 거짓인데 왔다 → 정책 결정 */
    LOG_WRN("notif_uri ignored: notif_enable=0 (%s/%d)", __func__, __LINE__);
}
```

> [!TIP] "조건 거짓인데 왔다" 는 규격서를 확인하고 **한 번만 정한다**
> 엄격하게 `RET_INVALID_ARG` 로 거절할지, 관대하게 로그만 남기고 무시할지는 규격과 상대 구현에 따라 다르다.
> 정해 두지 않으면 **함수마다 다르게** 처리하게 된다. `validate` 한 곳에서만 결정한다.

### 2-8. C 가 **여러 개**일 때 — 조건 문장의 모양을 먼저 분류한다

C 가 여러 개라고 무조건 union 이 아니다. **함께 존재할 수 있는가**로 갈린다.

```mermaid
flowchart TD
    Q1{"C 필드들이 <b>동시에</b><br/>존재할 수 있나?"}
    Q1 -->|"예 — 서로 무관"| P1["① 독립 조건<br/>각자 has_ + 각자 조건식<br/><b>union ✗</b>"]
    Q1 -->|"예 — 같이 오거나 같이 없음"| P3["③ 묶음<br/>하위 struct 로 감싸고 has_ 하나"]
    Q1 -->|"예 — 하나 이상이면 됨"| P4["④ 최소 하나<br/>각자 has_ +<br/>validate 에서 합계 ≥ 1"]
    Q1 -->|"아니오 — 정확히 하나만"| Q2{"어느 것인지 알려 주는<br/>필드(판별자)가 있나?"}
    Q2 -->|"있다 — 예: nf_type"| P2["② tagged union<br/>기존 enum 을 태그로 재사용"]
    Q2 -->|"없다"| P2b["② tagged union<br/>kind enum 을 새로 추가"]
```

| 패턴 | 규격서의 문장 예 | C 표현 |
| :--- | :--- | :--- |
| ① 독립 | "`a` 는 X 일 때, `b` 는 Y 일 때 present" (X·Y 는 서로 무관) | 각자 `has_` + 조건 함수 |
| ② **배타** | "`nfType` 이 AMF 면 `amfInfo`, SMF 면 `smfInfo`, UPF 면 `upfInfo`" | **tagged union** |
| ③ 묶음 | "`fqdn` 과 `port` 는 함께 present" | 하위 struct + `has_` 하나 |
| ④ 최소 하나 | "`ipv4Addr` 또는 `ipv6Addr` 중 **최소 하나**" | 각자 `has_` + `has_v4 + has_v6 >= 1` 검사 |

> [!DANGER] 동시에 올 수 있는 필드를 union 에 넣지 말 것
> union 은 **멤버들이 같은 메모리를 겹쳐 쓴다.** 하나를 쓰면 다른 하나가 **지워진다.**
> ④ 의 `ipv4` / `ipv6` 처럼 **둘 다 올 수 있는데** union 으로 만들면,
> 둘째를 파싱하는 순간 첫째가 조용히 망가진다. 컴파일도 되고 에러도 안 난다.
> **union 은 "정확히 하나만 존재한다"고 규격이 보장할 때만.**

### 2-9. 배타 C → **tagged union** (판별자 + union)

규칙은 하나다. **"지금 어느 멤버가 유효한가"를 알려주는 태그(판별자)를 union 과 함께 둔다.**
태그 없는 union 은 읽는 쪽이 어느 멤버를 봐야 하는지 **알 방법이 없다.**

**규격서 표** (단순화한 예)

| 필드 | 타입 | P | 조건 |
| :--- | :--- | :---: | :--- |
| `nf_type` | enum | **M** | AMF / SMF / UPF … |
| `amf_info` | object | **C** | `nf_type == AMF` 일 때 |
| `smf_info` | object | **C** | `nf_type == SMF` 일 때 |
| `upf_info` | object | **C** | `nf_type == UPF` 일 때 |

세 C 는 **`nf_type` 이 하나로 정하므로 동시에 존재할 수 없다** → 배타 → union.
그리고 **판별자가 이미 M 필드(`nf_type`)로 있다** → 새 태그를 만들 필요 없이 그걸 쓴다.

```c
/* ── 멤버마다 자기 구조체로 감싼다 ─────────────────────── */
typedef struct {                        /* AMF: 값이 한 개씩 */
    uint16_t  set_id;
    uint8_t   region_id;
} amf_info_t;

typedef struct {                        /* SMF: 값이 한 개씩 */
    uint16_t  tac;
    char      dnn[DNN_SIZE];
} smf_info_t;

typedef struct {                        /* UPF: ★ 여러 개 — 배열과 개수를 이 안에 가둔다 */
    uint8_t   has_fqdn;
    uint8_t   sst_cnt;
    uint8_t   sst[MAX_UPF_SST];
    char      fqdn[FQDN_SIZE];
} upf_info_t;

/* ── 본 구조체 ──────────────────────────────────────────── */
typedef struct {
    nf_type_t  nf_type;                 /* ★ 태그: 아래 union 중 무엇이 유효한지 */

    union {                             /* ★ nf_type 이 가리키는 것 하나만 유효 */
        amf_info_t  amf;
        smf_info_t  smf;
        upf_info_t  upf;
    } info;

    char       nf_id[NF_ID_SIZE];       /* 정렬 1 → 맨 끝 (4장) */
} nf_profile_t;
```

> [!NOTE] "C 가 한 개씩 / 여러 개" 가 섞여 있으면 **구조체로 감싸면 된다**
> `upf_info_t` 는 배열(여러 개)을 갖고, `amf_info_t` 는 단일 값만 갖는다.
> 이 차이를 **각자의 구조체 안에 가두면** union 입장에서는 전부 "구조체 하나"라서 **똑같이 다룰 수 있다.**
> 배열의 `cnt` 도 그 구조체 **안에** 넣는다. union 밖에 두면 태그가 바뀔 때 엉뚱한 개수를 보게 된다.
> 멤버가 단일 값 하나뿐이어도 (예: `uint16_t` 하나) **구조체로 감싸 두면** 나중에 필드가 늘어날 때 union 을 안 건드린다.

**읽기와 쓰기는 항상 태그를 거친다**

```c
/* 읽기: 태그가 맞을 때만 포인터를 내준다. 틀리면 NULL — 쓰레기를 읽을 기회를 아예 안 준다 */
const upf_info_t *nf_profile_get_upf(const nf_profile_t *p)
{
    if (p == NULL || p->nf_type != NF_TYPE_UPF) return NULL;
    return &p->info.upf;
}

/* 쓰기: ① 태그 ② union 전체를 0 으로 ③ 그 멤버만 채운다 */
int nf_profile_set_upf(nf_profile_t *p, const upf_info_t *src)
{
    CHK_PTR(p);
    CHK_PTR(src);

    p->nf_type = NF_TYPE_UPF;
    memset(&p->info, 0x00, sizeof(p->info));    /* ★ 이전 멤버의 잔재 제거 */
    p->info.upf = *src;
    return RET_OK;
}
```

**`validate` 는 유효한 멤버 하나만 본다**
...
case NF_TYPE_AMF:
    ...
    break;
case NF_TYPE_SMF:
    if (p->info.smf.dnn[0] == '\0') {
        LOG_ERR("smf dnn missing (%s/%d)", __func__, __LINE__);
        return RET_INVALID_ARG;
    }
    break;
case NF_TYPE_UPF:
    if (p->info.upf.sst_cnt == 0 || p->info.upf.sst_cnt > MAX_UPF_SST) {
        LOG_ERR("upf sst_cnt(%u) invalid (%s/%d)", p->info.upf.sst_cnt, __func__, __LINE__);
        return RET_INVALID_ARG;
    }
    break;
default:                                /* PCF 등 info 가 없는 NF — 조건에 해당 없음 */
    break;
}
```

> [!IMPORTANT] "조건은 참인데 `xxx_info` 가 안 왔다" 는 **파싱 단계**에서 잡는다
> tagged union 구조체 안에는 "안 왔다"를 나타낼 자리가 없다. 태그가 UPF 이면 `info.upf` 는 늘 "있는" 것이다.
> 그러므로 **JSON이나 바이트를 읽는 함수**가 `nf_type == UPF` 인데 `upfInfo` 가 없으면
> **그 자리에서 `RET_INVALID_ARG`** 를 내고 구조체를 채우지 않는다.
> 조건이 참일 때도 **그 멤버가 있을 수도 없을 수도 있다면** `uint8_t has_info;` 를 태그 옆에 하나 더 둔다.

**판별자가 따로 없는 배타 C — kind enum 을 새로 만든다**

```c
/* 규격: "주소는 ipv4 또는 ipv6 중 정확히 하나" — 이걸 알려 주는 필드가 표에 없다 */
typedef enum {
    ADDR_KIND_NONE = 0,                 /* ★ 0 = 아직 안 채움 (memset 과 맞아떨어진다) */
    ADDR_KIND_V4,
    ADDR_KIND_V6,
    ADDR_KIND_END
} addr_kind_t;
...
typedef struct {
    addr_kind_t kind;                   /* ★ 태그 */
    union {
        char v4[IPV4_STR_SIZE];
        char v6[IPV6_STR_SIZE];
    } u;
} addr_t;
```

### 2-10. tagged union 규칙 6개

| # | 규칙 | 어기면 |
| :---: | :--- | :--- |
| 1 | **태그가 항상 짝으로 있다.** 태그 없는 union 은 만들지 않는다 | 어느 멤버가 유효한지 알 수 없다 |
| 2 | 읽기 전에 **태그부터 확인** (`switch (tag)` 또는 getter) | 다른 멤버의 바이트를 **엉뚱한 타입으로 해석**한 쓰레기 값 |
| 3 | 쓸 때는 **태그 설정 → `memset(&u, 0)` → 멤버 채우기** 를 setter 함수 하나로 | 이전 멤버의 잔재가 남아 새 멤버 일부가 오염된다 |
| 4 | 태그의 **0 은 `NONE` / `UNKNOWN`** — `memset(0)` 초기화가 "아무것도 없음" 이 된다 | 초기화 안 한 구조체가 첫 멤버를 가진 것처럼 보인다 |
| 5 | `switch` 에 **`default:`** 를 둔다 (범위 밖 태그, info 가 없는 타입) | 알 수 없는 태그가 조용히 통과한다 |
| 6 | 멤버 안에 **동적 할당(포인터)** 이 있으면 `free` 도 **태그로 갈라서** 한다 | 잘못된 멤버의 포인터를 `free` → 크래시 또는 누수 |
...
void nf_profile_free(nf_profile_t *p)
{
    if (p == NULL) return;
    switch (p->nf_type) {
    case NF_TYPE_UPF:  SAFE_FREE(p->info.upf.big_list);  break;   /* 이 멤버가 포인터를 가질 때만 */
    default: break;
    }
...
> [!WARNING] union 의 **크기는 가장 큰 멤버**를 따른다
> `upf_info_t` 가 수 KB 이고 `amf_info_t` 가 4바이트여도 `nf_profile_t` 는 **항상** 수 KB 를 차지한다.
> - 멤버 크기가 **비슷하면** → union 이 이득 (합이 아니라 최댓값이니까)
> - **한 멤버만 유독 크면** → 그 멤버는 union 에서 빼고 **포인터**(`upf_info_t *upf`)로 두고, 태그가 UPF 일 때만 할당·해제한다

> [!NOTE] union 의 정렬은 **정렬이 가장 큰 멤버**를 따른다
> 4장의 배치 규칙에서 union 은 중첩 struct 와 같은 취급이다. 8바이트 멤버가 하나라도 있으면 union 전체가 8바이트 그룹이다.

> [!DANGER] union 을 통째로 `memcpy` 로 전송하지 말 것
> 전송 바이트열에는 **태그가 가리키는 멤버만** 써야 한다. 안 쓰는 멤버 자리의 바이트는 쓰레기이고,
> 상대와 union 크기도 다를 수 있다. **`pack` 함수에서 `switch (tag)` 로 갈라 필드를 하나씩** 쓴다.
> → [3번 문서](3.%20바이트%20골격%20—%20비트플래그·엔디안·Pack·Unpack.md)
...
typedef struct {
    ...
    /* Conditional ① 조건이 하나뿐 — 구조는 Optional 과 같고 validate 에서 조건을 본다 */
    uint8_t         has_{{조건필드}};
    {{조건타입}}    {{조건필드}};

    /* Conditional ② 배타 (정확히 하나만) — 태그 + union. 동시에 올 수 있으면 쓰지 말 것 */
    union {
        {{타입A}}_info_t  {{a}};        /* 태그 == {{TAG_A}} 일 때만 유효 */
        {{타입B}}_info_t  {{b}};        /* 여러 값을 갖는 쪽은 배열·cnt 를 자기 struct 안에 */
    } info;                             /* ★ 태그는 위쪽 {{필수enum}} 을 재사용하거나 kind 를 새로 둔다 */
} {{모듈}}_req_t;
...
/* ── union 이 있으면 반드시 짝으로 (직접 접근 금지) ─── */
const {{타입A}}_info_t *{{모듈}}_get_{{a}}(const {{모듈}}_req_t *r);   /* 태그 다르면 NULL */
int   {{모듈}}_set_{{a}}({{모듈}}_req_t *r, const {{타입A}}_info_t *src);
...
int {{모듈}}_req_validate(const {{모듈}}_req_t *r)
{
    ...
    /* ── Conditional ①: 조건이 참일 때만 필수 ── */
    if ({{조건식}}) {
        if (!r->has_{{조건필드}}) {
            LOG_ERR("{{조건필드}} required when {{조건설명}} (%s/%d)", __func__, __LINE__);
            return RET_INVALID_ARG;
        }
        /* 여기서 범위 검사 */
    } else if (r->has_{{조건필드}}) {
        LOG_WRN("{{조건필드}} ignored (%s/%d)", __func__, __LINE__);   /* 또는 거절 — 정책 */
    }

    /* ── Conditional ②: union 은 태그가 가리키는 멤버만 ── */
    switch (r->{{필수enum}}) {
    case {{TAG_A}}:  /* r->info.{{a}} 만 검사 */  break;
    case {{TAG_B}}:  /* r->info.{{b}} 만 검사 */  break;
...
| 규격서의 **C 를 O 로** 옮겨 적음 | 필수여야 할 필드가 빠져도 통과한다 | 표에 **조건 열**을 따로 둔다 → 2-6 |
| 동시에 올 수 있는 필드를 union 에 | 둘째를 넣는 순간 첫째가 **조용히 지워진다** | "정확히 하나" 일 때만 union → 2-8 |
| 태그 없는 union | 읽는 쪽이 어느 멤버인지 모른다 | 태그를 **항상 짝으로** → 2-10 |
| union 멤버를 바꿀 때 `memset` 안 함 | 이전 멤버 잔재가 남아 일부 오염 | setter 하나로 태그·`memset`·대입 묶기 |
| union 을 그대로 `memcpy` 해서 전송 | 안 쓰는 멤버 자리는 쓰레기 | `pack` 에서 `switch (tag)` 로 갈라 쓰기 |
...
- [ ] 각 필드의 **P(M/O/C)** 를 표에 적었는가
- [ ] **C 인 필드는 조건 문장**을 표의 조건 열에 옮겨 적었는가
- [ ] C 가 여러 개면 **동시에 올 수 있는지** 따져 union / 독립 / 묶음 / 최소하나를 골랐는가
...
- [ ] union 마다 **태그**가 짝으로 있고, 태그 0 이 `NONE` 인가
- [ ] union 접근이 **getter/setter 를 거치는가** (직접 접근하는 곳이 없는가)
...
### 구조체를 다 쓴 뒤
- [ ] `validate` 가 규격서의 **M/O/C**·범위·길이를 **전부** 반영하는가
- [ ] C 의 **조건이 거짓인데 값이 온 경우**의 정책을 정했는가 (거절 / 무시+로그)
- [ ] union 을 보는 `switch` 마다 **`default:`** 가 있는가
...
- [[Lang] Union, Typedef, Struct 구조 및 활용](../%5BLang%5D%20Union,%20Typedef,%20Struct%20구조%20및%20활용.md) — 2-9 tagged union 의 문법 기초
