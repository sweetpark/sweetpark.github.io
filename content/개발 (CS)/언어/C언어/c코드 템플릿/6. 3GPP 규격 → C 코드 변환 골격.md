...
| **C** 가 **여럿** (`oneOf`/`CHOICE`) | 정확히 하나 | **tagged union** → 2-4 |
...
### 2-4. `oneOf` / `anyOf` / CHOICE — C 가 **여럿**일 때

IE 표의 C 는 **"조건이 참이면 필수"** 한 종류가 아니다. OpenAPI(`oneOf`/`anyOf`/`allOf`)나 ASN.1 `CHOICE` 로 쓰인 규격은
**여러 필드의 존재 관계**를 말한다. 이걸 못 읽고 전부 `has_` 로 옮기면 **"둘 다 왔는데 통과"** 가 생긴다.

| 규격 표기 | 뜻 | C 표현 | 검사 |
| :--- | :--- | :--- | :--- |
| 조건 문장 ("…이면 present") | 다른 필드 값이 켜는 필수 | `has_` + 조건식 | 조건 참인데 없으면 에러 |
| **`oneOf`** / `CHOICE` | **정확히 하나** | **tagged union** | **0개도 에러, 2개 이상도 에러** |
| **`anyOf`** | **하나 이상** (여럿 가능) | 각자 `has_` | `has_a + has_b >= 1` |
| `allOf` | 전부 합친 것 | 그냥 멤버를 합친 구조체 | 각 멤버 규칙 그대로 |
| "A 와 B 는 함께 present" | 묶음 | 하위 struct + `has_` 하나 | 한쪽만 오면 에러 |

> [!DANGER] `anyOf` 를 union 으로 만들지 말 것
> `anyOf` 는 **둘 다 올 수 있다.** union 은 같은 메모리를 겹쳐 쓰므로 둘째를 파싱하는 순간 첫째가 **에러 없이 지워진다.**
> **`oneOf` 만 union.** 규격서에 `oneOf` 인지 `anyOf` 인지 **원문 단어를 확인**한다. 헷갈리면 `has_` 로 두는 쪽이 안전하다.

**`oneOf` → tagged union** (단순화한 예: 가입자 식별자는 `supi` 또는 `gpsi` 중 정확히 하나)

```c
typedef enum {
    UE_ID_NONE = 0,                         /* ★ 0 = 아직 안 채움 */
    UE_ID_SUPI,
    UE_ID_GPSI,
    UE_ID_END
} ue_id_kind_t;
...
typedef struct {
    ue_id_kind_t  e_kind;                   /* ★ 태그 (판별자가 규격에 없으니 내가 만든다) */
    union {
        char      sz_supi[SUPI_SIZE];
        char      sz_gpsi[GPSI_SIZE];
    } u;
} ue_id_t;
```

```c
/* oneOf: 표에 판별자 필드가 없으면 "어느 키가 왔나" 로 태그를 정한다 */
static int ue_id_parse(json_t *root, ue_id_t *out)
{
    json_t     *j_supi = NULL;              /* borrowed — decref 금지 */
    json_t     *j_gpsi = NULL;
    const char *s      = NULL;
    int         n_hit  = 0;
    ...
    j_supi = json_object_get(root, "supi");
    j_gpsi = json_object_get(root, "gpsi");
    n_hit  = (j_supi != NULL) + (j_gpsi != NULL);

    if (n_hit != 1) {                       /* ★ 0개도 2개도 위반 */
        LOG_ERR("ueId oneOf violated: %d present (%s/%d)", n_hit, __func__, __LINE__);
        return RET_BAD_FORMAT;
    }

    if (j_supi != NULL) {
        if (!json_is_string(j_supi)) return RET_BAD_FORMAT;
        s = json_string_value(j_supi);
        if (strlen(s) >= sizeof(out->u.sz_supi)) return RET_BAD_FORMAT;   /* ★ 길이 먼저 */
        snprintf(out->u.sz_supi, sizeof(out->u.sz_supi), "%s", s);
        out->e_kind = UE_ID_SUPI;           /* ★ 채운 뒤 태그 */
    } else {
        if (!json_is_string(j_gpsi)) return RET_BAD_FORMAT;
        s = json_string_value(j_gpsi);
        if (strlen(s) >= sizeof(out->u.sz_gpsi)) return RET_BAD_FORMAT;
        snprintf(out->u.sz_gpsi, sizeof(out->u.sz_gpsi), "%s", s);
        out->e_kind = UE_ID_GPSI;
    }
    return RET_OK;
}
```

> [!IMPORTANT] 생성(직렬화)도 **태그로 갈라서** 한다
> `switch (p->e_kind)` 로 해당 키 **하나만** 만든다. 안 쓰는 arm 의 바이트를 읽어 JSON 에 넣으면 쓰레기가 나간다.
> `default:` 는 `RET_INVALID_ARG` — 태그가 `NONE` 인 채로 응답을 만들면 **규격 위반**이다.

**판별자가 규격에 이미 있는 경우** (예: `nfType` 이 `AMF` 이면 `amfInfo`) 는 **그 필드를 태그로 재사용**한다.
태그가 둘이면 서로 어긋나는 순간이 온다. → [2번 노트 2-9](2.%20데이터%20모델%20골격%20—%20규격서%20I·O%20표를%20구조체로.md) 에 전체 골격과 규칙 6개.
...
> [!NOTE] ASN.1 `CHOICE` 는 **이미 tagged union 으로 생성된다**
> `asn1c` 가 만든 CHOICE 구조체는 **`present` (태그) + `choice` (union)** 형태다.
> 값 `Xxx_PR_NOTHING` (= 0) 이 "아직 안 정해짐" 이고, 내 구조체로 옮길 때는 **`switch (p->present)`** 로 arm 을 갈라 읽는다.
> `NOTHING` 을 그냥 통과시키면 안 된다 — M 인 CHOICE 라면 **반려**한다. 2-4 의 규칙이 그대로 적용된다.
...
| `oneOf` 를 `has_` 두 개로 옮기고 개수를 안 셈 | **둘 다 왔는데 통과 / 하나도 없는데 통과** | 키 개수를 세서 `!= 1` 이면 반려 (2-4) |
| `anyOf` 를 union 으로 옮김 | 둘째 파싱 시 첫째가 **조용히 지워짐** | `anyOf` 는 각자 `has_` + 합계 ≥ 1 |
| union 을 태그 확인 없이 읽음 / 생성 시 안 갈라 씀 | 안 쓴 arm 의 쓰레기가 응답에 실림 | 읽기·쓰기 모두 `switch (tag)` + `default:` |
...
- [ ] 규격 원문이 `oneOf` / `anyOf` / `allOf` / `CHOICE` 중 **무엇인지 단어로 확인**했나
- [ ] `oneOf` 는 tagged union + **0개·2개 이상 모두 반려**, `anyOf` 는 `has_` + 합계 검사인가
- [ ] `0..N` 배열에 **운용 상한**을 정했나
