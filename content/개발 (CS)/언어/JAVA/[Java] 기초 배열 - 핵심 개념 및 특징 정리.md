...
Java 배열의 선언·초기화, 객체 배열, 깊은/얕은 복사, 다차원 배열, 그리고 `ArrayList`의 기초를 예제로 정리한 노트다. 핵심은 배열이 고정 길이·연속 구조라 크기를 못 늘리는 한계가 있고, `System.arraycopy`는 객체 배열에서 참조만 복사하는 얕은 복사라는 점, 그리고 이런 한계를 보완해 크기 가변적이고 `get()`/`add()`로 다루는 `ArrayList`를 함께 다룬다는 점이다.

> [!NOTE] 실행 환경
> 제네릭 `ArrayList<T>`와 향상된 for문이 사용되어 최소 Java 5 이상 환경을 전제로 한다. 구체적인 JDK 마이너 버전은 코드상 명시되어 있지 않다.
