...
순수 Servlet만으로 회원 등록(form)과 저장(save) 기능을 구현한 예제다. 핵심은 `PrintWriter`로 자바 코드 안에서 직접 HTML 문자열을 조립해 응답하는 방식이며, 이 방식은 데이터를 다루기는 쉽지만 HTML과 로직이 섞여 코드가 길어지고 가독성·유지보수성이 떨어진다는 한계가 있다. 이 한계가 이후 템플릿 엔진(JSP 등) 도입의 동기가 된다.

> [!NOTE] 실행 환경
> `@WebServlet` 어노테이션 기반 등록 방식이 사용되어 Servlet 3.0 이상을 전제로 하며, Tomcat 등 Servlet 컨테이너 실행 환경이 필요하다. 구체적인 Servlet/Tomcat 버전은 코드상 명시되어 있지 않다.
